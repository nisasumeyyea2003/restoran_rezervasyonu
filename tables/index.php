...
<div class="section py-5">
    <div class="container">
        <h3 class="text-center"><b>Table List</b></h3>
        <hr>
        <div class="row justify-content-center">
            <div class="col-lg-8 col-md-10 col-sm-12 col-sm-12 mb-3">
                <div class="input-group input-group-lg" id="search-field">
                    <input type="search" class="form-control form-control-lg  rounded-pill" aria-label="Search table Field" id="search" placeholder="Search table here">
        ...
        <div class="accordion" id="tableAccordion">
            <?php 
            $tables = $conn->query("SELECT * FROM `table_list` where delete_flag = 0 and `status` = 1 order by `name` asc");
            while($row = $tables->fetch_assoc()):
            ?>
            <div class="card mb-0 rounded-0">
                <div class="card-header" id="table<?= $row['id'] ?>">
                    <h2 class="mb-0">
                ...
                <div id="collapse<?= $row['id'] ?>" class="collapse table_collapse" aria-labelledby="table<?= $row['id'] ?>" data-parent="#tableAccordion">
                    <div class="card-body">
